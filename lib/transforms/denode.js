...
module.exports = function(mod, ast, opts) {
  ...
  for (var i = 0, len = requires.length; i < len; i++) {
    ...
    if (mod.resolve.isCore(dep)) {
      ...
      var filename = mod.resolve(dep);
      var ext = path.extname(filename);
      var aid = path.relative(baseDir, filename);
  ...
  estraverse.traverse(ast, {
    leave: function(node) {
      if (isRequire(node)) {
        ...
        deps = node.arguments[di].elements;
        var elements = deps.map(function(el) {
          var mid = map[el.value] || el.value;
