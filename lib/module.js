...
Module.get = function(id, filename) {
  var cachedModule = Module._cache[filename];
  if (cachedModule) { return cachedModule; }
  return new Module(id, null);
}
...
Module._load = function(id, parent, opts, cb) {
  ...
  try {
    ...
    // This only applies to absolute module IDs.  Unresolved relative module IDs
    // indicate an internal issue with the module itself.
    if ((opts.allowUnresolved && id[0] != '.') || opts.allowAllUnresolved) {
  ...
  if (parent.resolve.isBuiltIn(id)) {
    // Built-in modules are either provided by the loader or compiled into the
    // JavaScript environment (such as `require`, `exports`, and `module` in AMD
    // and `events`, `path, `net`, etc. in Node.js)
    return cb(null, null);
  }
...
Module.prototype.toAMD = function(opts) {
  ...
  utils.merge(genopts, opts.generate || {});
